...
//                        .setErrorText("https://issues.jboss.org/browse/JBEAP-2686"),
       ...
       new FilterItem(Colour.YELLOW)
               .addTestMatcher((JSONObject errorDetails) -> errorDetails.get("errorDetails").equals("Thread leaked"))
               .setErrorText("Thread leaked"),
       new FilterItem(Colour.YELLOW)
               .addTestMatcher((JSONObject errorDetails) -> errorDetails.get("errorDetails").equals("broadcast not received"))
               .setErrorText("broadcast not received"),
       new FilterItem(Colour.YELLOW)
               .addTestMatcher((errorDetails) -> Pattern.compile("(libAIO is not loaded).*").matcher((CharSequence) errorDetails.get("errorDetails")).find())
               .setErrorText("libAIO is not loaded"),
       new FilterItem(Colour.RED_BOLD)
               .addTestMatcher((JSONObject errorDetails) -> errorDetails.get("errorDetails").equals("Didn\"t get the expected number of bindings, look at the logging for more information"))
               .setErrorText("Didn\"t get the expected number of bindings, look at the logging for more information"),
       new FilterItem(Colour.YELLOW)
               .addTestMatcher((JSONObject errorDetails) -> Pattern.compile("(AMQ119007:).*").matcher((CharSequence) errorDetails.get("errorDetails")).find())
